...
const MedicalExams = () => {
  ...
  const fetchExams = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/exams?category=Medical`);
      ...
      console.log('Medical Exams API Response:', data);
      ...
      if (examsList.length === 0) {
        setError('No medical exams found');
        return;
      }
  ...
  const fetchPapers = async () => {
    try {
      ...
      const data = await response.json();
      console.log('Medical Papers API Response:', data);
      ...
      console.error('Error fetching medical papers:', err);
  ...
  const getExamColor = (examTitle) => {
    ...
    return colorMap[examTitle] || 'bg-red-500';  // Default to red
  };
  ...
  const handleStartTest = (paper) => {
    ...
    navigate("/medical-exams/rules", { state: examDetails });
  };
